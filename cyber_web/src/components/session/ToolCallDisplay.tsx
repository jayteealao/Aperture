...
function BashDisplay({ input }: { input: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-2">
      {description && (
        <div className="flex items-center gap-2 mb-1 text-xs text-hud-text/50">
          <Terminal size={12} />
...
function FileDisplay({ path }: { path: string }) {
  return (
    <div className="mt-2 flex items-center gap-2 px-3 py-2 bg-hud-gray/20 border border-hud-gray/30">
      <File size={14} className="text-hud-text/50 shrink-0" />
      <code className="text-xs font-mono text-hud-text/70 truncate">
        {path}
      </code>
    </div>
  )
...
function WriteDisplay({ input }: { input: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-2 space-y-2">
      <div className="flex items-center gap-2 px-3 py-2 bg-hud-gray/20 border border-hud-gray/30">
        <File size={14} className="text-hud-text/50 shrink-0" />
        <code className="text-xs font-mono text-hud-text/70 truncate">
          {filePath}
        </code>
...
function WebFetchDisplay({ input }: { input: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-2 flex items-center gap-2 px-3 py-2 bg-hud-gray/20 border border-hud-gray/30">
      ...
      <code className="text-xs font-mono text-hud-accent truncate">
        {url}
      </code>
    </div>
  )
