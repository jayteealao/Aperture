...
function BashDisplay({ input }: { input: Record<string, unknown> }) {
  ...
  return (
    <div>
      {description && (
        <div className="flex items-center gap-2 mb-2 text-xs text-hud-text/50">
          <Terminal size={12} />
...
function FileDisplay({ path }: { path: string }) {
  return (
    <div className="flex items-center gap-2">
      <File size={14} className="text-hud-text/50 shrink-0" />
      <code className="text-[10px] font-mono text-hud-text/70 truncate">
        {path}
      </code>
    </div>
  )
...
function EditDisplay({ input }: { input: Record<string, unknown> }) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <Edit3 size={14} className="text-hud-text/50 shrink-0" />
        <code className="text-[10px] font-mono text-hud-text/70 truncate">
          {filePath}
        </code>
      </div>
      {(oldString || newString) && (
        <div className="rounded overflow-hidden border border-hud-gray/30">
          {oldString && (
            <div className="bg-hud-error/10 border-b border-hud-gray/30">
              <div className="px-2 py-1 text-2xs font-medium text-hud-error border-b border-hud-gray/30">
                − Remove
              </div>
              <pre className="p-2 text-[10px] overflow-x-auto text-hud-text/70">
                {truncate(oldString, 150)}
              </pre>
            </div>
          )}
          {newString && (
            <div className="bg-hud-success/10">
              <div className="px-2 py-1 text-2xs font-medium text-hud-success border-b border-hud-gray/30">
                + Add
              </div>
              <pre className="p-2 text-[10px] overflow-x-auto text-hud-text/70">
...
function SearchDisplay({ input }: { input: Record<string, unknown> }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <Search size={14} className="text-hud-text/50 shrink-0" />
      <div className="text-[10px] overflow-hidden">
        <code className="font-mono text-hud-accent">{pattern}</code>
        {path && (
          <span className="text-hud-text/50"> in {path}</span>
        )}
      </div>
    </div>
  )
...
function WebFetchDisplay({ input }: { input: Record<string, unknown> }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <Globe size={14} className="text-hud-text/50 shrink-0" />
      <code className="text-[10px] font-mono text-hud-accent truncate">
        {url}
      </code>
    </div>
  )
...
function WebSearchDisplay({ input }: { input: Record<string, unknown> }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <Search size={14} className="text-hud-text/50 shrink-0" />
      <span className="text-[10px] text-hud-text/70">
        Searching: <span className="text-hud-accent font-medium">{query}</span>
      </span>
    </div>
  )
}
...
function TaskDisplay({ input }: { input: Record<string, unknown> }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <Bot size={14} className="text-hud-text/50 shrink-0" />
      {subagentType && (
      ...
      {description && (
        <span className="text-[10px] text-hud-text/70">{description}</span>
      )}
    </div>
  )
}
