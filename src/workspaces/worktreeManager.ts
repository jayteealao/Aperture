  ...
  private async ensureNativeModule(): Promise<NativeAddonModule> {
    if (!this.nativeModule) {
      try {
        ...
        // Import .js file (generated by napi-rs build) not .ts source file
        // Use URL.href for ESM import() which requires file:// URLs on Windows
        const nativeUrl = new URL('../../packages/worktrunk-native/index.js', import.meta.url);
        this.nativeModule = await import(nativeUrl.href) as NativeAddonModule;
      } catch (error) {
        throw new Error(
