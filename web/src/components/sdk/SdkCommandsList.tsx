...
export function SdkCommandsList({
  ...
  if (needsPrompt) {
    return (
      <div className="text-center py-3">
        <Terminal size={24} className="mx-auto text-foreground/40 mb-2" />
        <p className="text-xs text-foreground/40">Send a prompt to load commands</p>
      </div>
    )
  }
  ...
  if (commands.length === 0) {
    return (
      <div className="text-center py-3">
        <Terminal size={24} className="mx-auto text-foreground/40 mb-2" />
        <p className="text-xs text-foreground/40">No commands available</p>
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-1 max-h-[300px] overflow-y-auto scrollbar-thin">
        {filteredCommands.map((cmd) => (
          ...
          >
            ...
            <p className="text-2xs text-foreground/40 mt-0.5 line-clamp-2">
              {cmd.description}
            </p>
          </button>
        ))}
      ...
      <div className="text-2xs text-foreground/40 text-center">
